import React from 'react';
import MobileTool from '../Components/MobileTool';
import PageHeaderBack from '../Components/PageHeaderBack';
import NotificationFilterChips from '../Components/NotificationFilterChips';
import NotificationCard from '../Components/NotificationCard';
import './Notifications.css';

const GearIcon = () => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden>
        <path
            d="M12 15a3 3 0 100-6 3 3 0 000 6z"
            stroke="currentColor"
            strokeWidth="1.6"
            strokeLinecap="round"
        />
        <path
            d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2h-1a2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2v-1a2 2 0 012-2h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06a1.65 1.65 0 001.82.33H9a1.65 1.65 0 001-1.51V3a2 2 0 012-2h1a2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06a1.65 1.65 0 00-.33 1.82V9a1.65 1.65 0 001.51 1H21a2 2 0 012 2v1a2 2 0 01-2 2h-.09a1.65 1.65 0 00-1.51 1z"
            stroke="currentColor"
            strokeWidth="1.2"
            strokeLinecap="round"
            strokeLinejoin="round"
        />
    </svg>
);

const NotificationsPage = () => (
    <div className="notifications-page">
        <MobileTool />
        <div className="notifications-page__inner">
            <PageHeaderBack
                title="NOTIFICATIONS"
                right={
                    <button type="button" className="notifications-page__gear" aria-label="Settings">
                        <GearIcon />
                    </button>
                }
            />
            <NotificationFilterChips />
            <NotificationCard
                variant="transfer"
                letter="S"
                name="Sarah Abdelrahman"
                amount="+500$"
                body="500$ transfered to your wallet"
                time="20 mins ago"
            />
            <NotificationCard
                variant="referral"
                letter="G"
                name="Gain money by refering"
                body="Refer the app to 3 people to get 100$ extra"
                time="30 mins ago"
            />
            <NotificationCard
                variant="referral"
                letter="G"
                name="Gain money by refering"
                body="Refer the app to 3 people to get 100$ extra"
                time="30 mins ago"
            />
        </div>
    </div>
);

export default NotificationsPage;
